...
const UpdateTimeslotHospitalAdmin = () => {
    ...
    const handleConfirm = () => {
        window.location.href = `/super-admin/timeslothospital/`
    }

    const handleSubmitDate = async () => {
        // Fetch timeslots based on the selected date
        try {
            const response = await fetch('http://localhost:5000/super-admin-get-timeslothospital', {
                method: 'POST',
                headers: {
    ...
    const handleSlotChange = async (hosSlotID, newAmount) => {
        try {
            ...
            const response = await fetch('http://localhost:5000/super-admin-update-timeslothospital', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('tokenSuperAdmin')}`,
            ...
            if (response.ok) {
                ...
                window.location.href = `/super-admin/timeslothospital/${selectedDate}`;
            } else {
                console.error('Failed to update timeslot hospital:', response.statusText);
                alert('Failed to update timeslot hospital');
            }
        } catch (error) {
            console.error('Error updating timeslot hospital:', error);
            alert('Error updating timeslot hospital');
        }
    };
...
export default UpdateTimeslotHospitalAdmin;
