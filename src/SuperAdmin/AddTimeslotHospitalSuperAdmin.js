...
const AddTimeslotHospitalSuperAdmin = () => {
    ...
    const fetchExistingYears = async () => {
        try {
            const response = await fetch('http://localhost:5000/super-admin-get-existing-years-timeslothospital', {
                method: 'GET',
                headers: {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await fetch('http://localhost:5000/super-admin-add-timeslothospital', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('tokenSuperAdmin')}`,
...
export default AddTimeslotHospitalSuperAdmin;
